    ...
    const fetchStats = async () => {
      try {
        ...
        if (json.success && json.data) {
          ...
          const unfeatured = stats.totalTutors - stats.totalFeatured;
          ...
          setData([
            { 
              ...
              percentage: ((stats.totalFeatured / total) * 100).toFixed(1)
            }, 
            { 
              name: "Standard Tutors", 
              value: unfeatured, 
              color: "#f59e0b", // Amber
              percentage: ((unfeatured / total) * 100).toFixed(1)
            }, 
          ]);
        }
      } catch (err) {
        console.error("Failed fetching featured stats:", err);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="bg-card border border-black/5 dark:border-white/5 rounded-2xl shadow-xs p-6 mb-8 w-full animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Current distribution of featured tutor placements.
        </p>
      </div>
